...
Cluster and OIDC server configuration files such as `podplane.cluster.jsonc` and `podplane.oidc.jsonc` are not covered here. See the [Config Files & Context](overview.md#config-files--context) section in the CLI Overview for more information about those files.
...
- **Dependencies cache**: dependency files are grouped by dependency domain under the dependencies cache root. VMConfig manifests and artifacts live under `deps/vmconfig/`, component manifests live under `deps/components/`, template manifests and cached chart OCI data live under `deps/templates/`, seed manifests/snapshots live under `deps/seeds/`, and cluster OpenTofu/Terraform provider mirrors, versioned modules, lock information, and the shared `podplane.tfrc` live under `deps/tf/`. Terraform dependency downloads are additive so clusters pinned to older versions can continue using the shared cache.
...
- **Local registry cache**: mirrored container image data for the node-local registry is stored under `registry/`. This shared registry store can be populated by dependency downloads and future commands that build or import local images. Safe to delete; images can be downloaded or rebuilt again.
