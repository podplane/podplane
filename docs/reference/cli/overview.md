...
The CLI uses the current working directory to find the relevant `podplane.cluster.jsonc` or `podplane.oidc.jsonc` config file. See [Configuration](../configuration.md) for the full file format documentation.
...
See [CLI Storage](storage.md) for more details about these files and how they are stored.
