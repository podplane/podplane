...
func (m *Local) Start(opts StartOptions) (string, error) {
	...
	mutableVars := clusterspec.MutableEnv{
		...
		"TELEMETRY_LOG_SERVICES":         "first-boot-env,cron,ssh,netsy,nstance-agent,nstance-recv-watch,containerd,kube-apiserver,kube-controller-manager,kube-scheduler,kubelet,registry",
...
`, componentsSource.URL, refKey, refValue, secretRefBlock)
	...
	vaultCACert := ""
	if ca, err := os.ReadFile(m.OIDCCACertPath()); err == nil {
		vaultCACert = strings.TrimSpace(string(ca))
	}
	path := ClusterConfigPath(m.dataDir, clusterID)
